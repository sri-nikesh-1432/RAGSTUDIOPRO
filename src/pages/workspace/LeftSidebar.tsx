...
import {
  ...
  Layers, Database, Search, MessageSquare, ChevronRight,
  Bookmark, Settings2, RefreshCcw
} from 'lucide-react';
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isDragOver ? 'border-accent-primary bg-accent-primary/5 scale-[1.02]' : config.border,
        config.bg, 'hover:bg-opacity-60'
      )}
    >
      ...
      <div className="flex items-center gap-3" onClick={() => fileInputRef.current?.click()}>
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-[9px] text-text-muted truncate mt-0.5">
            {isUploading ? uploadProgress : `${fileCount} file${fileCount !== 1 ? 's' : ''}`}
          </p>
        </div>
        {isUploading ? (
...
export function LeftSidebar({
  ...
  return (
    <div className="w-72 shrink-0 border-r border-border-primary bg-bg-secondary flex flex-col h-full">
      ...
      {/* Upload Zones */}
      <div className="shrink-0 p-3 space-y-2 border-b border-border-primary">
      ...
      {/* URL & Text Input */}
      <div className="shrink-0 px-3 py-2 border-b border-border-primary space-y-2">
        <button
        ...
        <textarea value={pasteText} onChange={(e) => { setPasteText(e.target.value); store.setCurrentText(e.target.value); }}
          ...
          className="w-full h-16 bg-bg-elevated rounded-lg border border-border-primary px-2.5 py-1.5 text-xs text-text-primary resize-none focus:outline-none focus:border-accent-primary transition-colors placeholder:text-text-muted" />
      </div>
      ...
      {/* Bottom Status */}
      <div className="shrink-0 px-4 py-2 border-t border-border-primary bg-bg-secondary/80">
        <div className="flex items-center justify-between text-[9px] text-text-muted">
          <span>Pipeline: Step {activeStep + 1}/7</span>
